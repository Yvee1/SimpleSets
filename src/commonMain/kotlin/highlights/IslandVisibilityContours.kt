...
fun Highlight.visibilityIntervals(other: Highlight): List<VisibilityInterval> {
    ...
    return (when(this) {
        ...
        // TODO: Check below
        is IslandHighlight -> circles.zip(arcs.map { it.contour }).mapNotNull { it.visibilityInterval(other) } + segments.mapNotNull { it.visibilityInterval(other) }
        is BankHighlight -> circles.zip(arcs.map { it.contour }).mapNotNull { it.visibilityInterval(other) } + segments.mapNotNull { it.visibilityInterval(other) }
        else -> error("Unknown highlight type")
    }).mergeIntervals()
}
